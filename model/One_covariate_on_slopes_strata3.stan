...
real B_SPECIES[nspecies]; // species level mean effect of covariate across all strata


//real<lower=0> sd_B_group; //sd on 
//real<lower=0> sd_B_species;
real<lower=0> sd_b_group;
...
transformed parameters{
  ...
  vector[nstrata] b_group; // group param
  
  matrix[nspecies,nstrata] b_species; // species param
 ...
 b_group = sd_b_group*b_raw_group + B_GROUP;
...
// Group level effects
b_raw_group ~ std_normal();
sum(b_raw_group) ~ normal(0,0.0001*nstrata);
B_GROUP ~ normal(0,0.1);
sd_b_group ~ gamma(2,0.1);//boundary avoiding prior // ~ normal(0,0.1);
...
// species effects
SPS ~ normal(0,0.1);
sd_sps ~ gamma(2,0.1);//boundary avoiding prior // ~ normal(0,0.1);


B_SPECIES ~ normal(0,0.1);
sum(B_SPECIES) ~ normal(0,0.0001*nspecies);

sd_b_species ~ gamma(2,0.1);//boundary avoiding prior // ~ normal(0,0.1);
for(s in 1:nspecies){
